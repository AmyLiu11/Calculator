...
+ (void)drawHashMarksInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
	...
	int unitsPerHashmark = MIN_PIXELS_PER_HASHMARK * 2 / pointsPerUnit;//正负半轴，所以要乘2
	...
	{
		BOOL drew = NO;
		CGFloat scaledOffset = floor(offset * pointsPerUnit);// The floor() functions return the largest integral value less than or equal to x.integral value整数值
 		CGPoint hashMarkPoint;
		hashMarkPoint.x = axisOrigin.x+scaledOffset;//描点，每次向前移动一个hashMark
		...
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
			...
			CGContextAddLineToPoint(context, hashMarkPoint.x, hashMarkPoint.y+HASH_MARK_SIZE);//此处画标度
			[self drawString:[NSString stringWithFormat:@"%d", offset] atPoint:hashMarkPoint withAnchor:ANCHOR_TOP];//此处画数字
			drew = YES;
		}//要判断该点在不view内，若不在则无须画标度和数字
...
{//axisOrigin为坐标原点
	...
	CGContextAddLineToPoint(context, bounds.origin.x+bounds.size.width, axisOrigin.y);//添加横坐标
	CGContextMoveToPoint(context, axisOrigin.x, bounds.origin.y);
	CGContextAddLineToPoint(context, axisOrigin.x, bounds.origin.y+bounds.size.height);//添加纵坐标
	CGContextStrokePath(context);
